...
local function is_punct(ch) return PUNCTS[ch] == true end

local function str_len_chars(line) return vim.fn.strchars(line) end
local function char_at(line, cidx)  return vim.fn.strcharpart(line, cidx - 1, 1) end
local function col0_to_charidx(line, col0) return vim.str_utfindex(line, col0) + 1 end
local function charidx_to_col0(line, cidx) return vim.str_byteindex(line, cidx - 1) end

local function token_bounds(line, cur)
  local len = str_len_chars(line)
  if len == 0 then return cur, cur end
  ...
  if ch ~= "" and is_space(ch) then
    local i = cur
    while i <= len and is_space(char_at(line, i)) do i = i + 1 end
    if i > len then return len, len end
    cur = i
    ch = char_at(line, cur)
  end
  ...
  while s > 1 do
    ...
    if is_space(prev) or is_punct(prev) then break end
    s = s - 1
  end
  local e = cur
  while e < len do
    local nxt = char_at(line, e + 1)
    if is_space(nxt) or is_punct(nxt) then break end
    e = e + 1
  end
  return s, e
end
...
local function next_token_start_global(row, e_cidx)
  ...
  local i = e_cidx + 1
  while i <= len and is_space(char_at(line, i)) do i = i + 1 end
  if i <= len then return row, i end
  ...
  while row <= lastrow do
    ...
    local j = 1
    while j <= len and is_space(char_at(line, j)) do j = j + 1 end
    if j <= len then return row, j end
    row = row + 1
  end
...
local function prev_token_end_global(row, s_cidx)
  ...
  while i >= 1 and is_space(char_at(line, i)) do i = i - 1 end
  if i >= 1 then
  ...
  while row >= 1 do
    ...
    len  = str_len_chars(line)
    local j = len
    while j >= 1 and is_space(char_at(line, j)) do j = j - 1 end
    if j >= 1 then
...
local function next_step_or_start(row, cidx)
  ...
  if i <= len then
    ...
    if not is_space(ch) then return row, i end
  end
